...
void GetImageFile(const std::string& image_file,
  ...
  std::cout<< "mean(r,g,b)" << mean_r << "," << mean_g << "," <<mean_b <<std::endl;
}
...
std::vector<std::string> LoadSynset(const std::string& synset_file) {
  ...
  while (fi >> synset) {
    ...
    // std::cout << "Added class " << lemma << std::endl;
  }
...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    std::cout << "No test image here, or no params here." << std::endl
              << "Usage: ./image-classification-predict apple.jpg ./model/cifar10-dev-0016.params" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::string json_file = "model/cifar10-symbol.json";
  std::string synset_file = "model/batches.meta.txt";
  std::string nd_file = "model/cifar_mean.bin";
  ...
  const mx_uint input_shape_data[4] = { 128,
  ...
  std::cout << dev_type  << " " << dev_id << " " << num_input_nodes << " " << *input_keys << " " << std::endl;
  assert(input_shape_indptr);
  assert(input_shape_data);
  
  // Create Predictor
  int  isPredSucceed = MXPredCreate(static_cast<const char*>(json_data.GetBuffer()),
               ...
               input_shape_data,
               &pred_hnd);
  std::cout << "pred_hnd:" << pred_hnd << " is succeed(0 when succeed):" << isPredSucceed << std::endl;
  assert(pred_hnd);
  ...
  std::vector<float> data_cut(10);
  for (int index = 0; index <10; ++index) { data_cut[index] = data[index]; }  

  // Print Output Data
  PrintOutputResult(data_cut, synset);
